...
class EntropyProduction(TimeEvolution):  
	...
	def calculate_entropy_real_space(self):
		reg = 1
	...
	def calculate_entropy_fourier_space(self): 
		reg = 100
		self.final_phi = self.phi[-2] 

		self._make_laplacian_fourier() 
		self._make_first_order_matrix_fourier() 
		self._make_noise_matrix_fourier() 
		self._add_to_translational_dof(reg=reg) 
		self._make_correlation_matrix() 
		self.correlation_matrix = sp.csr_matrix(self.correlation_matrix)
		...
		plt.plot(S_real.diagonal())
		plt.show() 
		self.entropy = S_real.diagonal() 

	def calculate_entropy_quad_bd(self): 
		reg = 1
		self.final_phi = self.phi[-2] 
	...
	def _make_first_order_matrix_fourier(self):
	...
	def _make_noise_matrix_fourier(self): 
